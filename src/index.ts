...
const rooms: Room = new Map<string, WebSocket[]>(); // Data structure to store the rooms + Users
...
wss.on("connection", (socket: WebSocket) => {
  ...
  socket.on("message", (message) => {
    try {
      ...
      if (data.type === "chat") {
        ...
        if (!room) {
          socket.send(
            "You need to Join a room first before sending any message",
          );
          return;
        }
  ...
  socket.on("close", () => {
    for (const [roomId, users] of rooms) {
      ...
      if (updatedUsers.length === 0) rooms.delete(roomId);
      else {
